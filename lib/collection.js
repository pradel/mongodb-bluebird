...
Collection.prototype.group = function (keys, condition, initial, reduce, finalize, command, options) {
    var resolver = Promise.defer();
    this._collection.group(keys, condition, initial, reduce, finalize, command, options, function(err, docs) {
        if (err)
            return resolver.reject(err);
        return resolver.resolve(docs);
    });
...
Collection.prototype.aggregate = function (array, options) {
    var resolver = Promise.defer();
    this._collection.aggregate(array, options, function(err, docs) {
        if (err)
            return resolver.reject(err);
        return resolver.resolve(docs);
    });
...
Collection.prototype.formatQuery = function(query) {
    for (var i = 0; i < this._objectIds.length; i++) {
        if (query && query[this._objectIds[i]]) {
